...
import type { Dice, GameState, PlayerState } from '../types/index.js';
...
export function gameReducer(state: GameState, action: GameAction): GameState {
  ...
  switch (action.type) {
    ...
    case 'BANK': {
      ...
      if (!canBank(state.turn, currentPlayer.isOnBoard)) {
    ...
    case 'END_TURN': {
      ...
      if (turnScore > 0) {
        ...
        const meetsEntryThreshold = ownScore >= ENTRY_THRESHOLD;
      ...
      const triggeredFinalRound = !state.isFinalRound && newScore >= TARGET_SCORE;
