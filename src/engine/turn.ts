 ...
 * - If not on the board, must have >= ENTRY_THRESHOLD points from own rolls (not carryover)
 * - Cannot bank immediately after claiming a carryover (must roll at least once more)
 */
export function canBank(state: TurnState, isOnBoard: boolean): boolean {
  ...
  // If not on board, need >= ENTRY_THRESHOLD from own score
  if (!isOnBoard) {
