...
export const aggressiveStrategy: AIStrategy = (turnState, isOnBoard) => {
  ...
  const minBankThreshold = isOnBoard ? 3500 : Math.max(3500, ENTRY_THRESHOLD);
...
export const balancedStrategy: AIStrategy = (turnState, isOnBoard) => {
  ...
  if (!isOnBoard && turnState.turnScore >= ENTRY_THRESHOLD) {
    ...
    // If we have a solid lead over threshold OR bust risk is significant, bank to secure entry
    // With 1 die: 67% bust chance - definitely bank
    // With 2 dice: 44% bust chance - bank if we have decent score
    // With 3+ dice: evaluate more carefully
    if (bustProb >= 0.4 || turnState.turnScore >= 800) {
      return { action: 'BANK' };
    }
  }
